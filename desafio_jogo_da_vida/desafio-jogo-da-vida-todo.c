...
#include<stdio.h>
#include<stdlib.h>
#define ARQUIVO_PADRAO "teste01.txt"
...
int main()
{
    ...
    // essa atribuição coleta os caracteres do arquivo selecionado no define e coloca em uma matriz "m".
...
void jogo(int **matriz, int nl, int nc, int iter)
{
    ...
    // uma copia é realizada para não haver interações entre o resultado e a matriz original
...
{
    int ** alocada, i;
    alocada = (int **) malloc (nl * sizeof(int *));
    for (i=0;i<nl;i++)
        alocada[i] = (int *) malloc (nc * sizeof(int));
