...
void realizarJogada(char **jogo, char **resultado, int *naviosRestantes, int lin, int col);
        ...
        if (x >= 0 && x < col && y >= 0 && y < lin){
            if (jogo[y][x] != '-')
                ...
                if (resultado[y][x] == 'N'){
                    ...
                    // Verifica se o navio foi completamente destru�do
                    ...
                    // Marca as posi��es como atingidas
                    if (navioDestruido){
                        for (int j = x; j < x + LARGURA_MAX; j++){
                            jogo[y][j] = 'X';
                            resultado[y][j] = 'X';
                    ...
                    jogo[y][x] = 'O';
                    resultado[y][x] = 'O';
                    continuaJogando = 0; // Encerra o loop, passa a vez para o pr�ximo jogador
                }

                // Atualiza o tabuleiro exibido apos cada tiro
                limparTela();
                printf("Seu ataque:\n");
...
int main(){
    ...
    while (1){
        if (vezDoJogador == 1){
            ...
            realizarJogada(jogador1, resultado2, &naviosRestantes2, lin, col);
            ...
            realizarJogada(jogador2, resultado1, &naviosRestantes1, lin, col);
